...
def angular_velocity( i, j_T_i, dt):
    return 0
...
def rodrigues(omega_hat, theta):
    ...
    R = I + s * omega_hat + (1-c)*(omega_hat @ omega_hat)
...
def animate(i, frames, quivers, configurations, dt):
    ...
    if move_i:
        ...
        j_p_i_dot = another_fake_algorithm_transl_vel(i, j_T_i)


        ##j_omega_i = j_T_i.T @i_omega_i
        j_EA_i = R2EA(j_T_i[:3, :3])  ##
        j_EA_i += i_omega_i_to_EA_dot(i_omega_i, j_EA_i) * dt##
        


        j_T_i[:3, :3] = EA2R(j_EA_i)
